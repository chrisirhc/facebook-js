var querystring = require('querystring'),
    ...
    function convertTime(time) {
      return new Date(time).getTime();
		...
		function stringifyEvent( eve ) {
			var str = '+{+end_time:+' + eve.end_time +
							  '+,+location:+\'' + eve.location.replace(/'/g, "\\'") +
								'\'+,+name:+\'' + eve.name.replace(/'/g, "\\'") +
								'\'+,+start_time:+' + eve.start_time +
								'+,+description:+\'' + eve.description.replace(/'/g, "\\'") +
								'\'+,+page_id:+\'' + eve.page_id +
...
module.exports = function (api_key, api_secret) {
  ...
  client.RSVP_NOREPLY = 'not_replied'; //haven't response to invitation
   ...
   * params: access_token, format,
   *	i.e. {access_token: token,
   *	 format: 'json'},
   * params: query array
   *  i.e. [ query1: 'SELECT uid, rsvp_status FROM event_member WHERE eid= XXXXXXX'
  ...
  client.rsvpEvent = function (eid, token, status, callback) {
    doRequest(
      'GET',
      facebook_api_url + '/method/events.rsvp?eid=' + eid + '&rsvp_status=' + status + '&access_token=' + token + '&format=json',
      callback
    );
  ...
  /**
   * Create event
   * param: token page_ownder's access token
   * param: eventObj 
   * { end_time: 1298134800000 , 
   * location: 'Georgia Tech' , 
   * name: 'Georgia Tech Event' ,
   *  start_time: 1298125800000 , 
   ...
   */
  client.createEvent = function(token, param, callback) {
    doRequest(
      'GET',
     	facebook_api_url + '/method/events.create?event_info=' + escape(stringifyEvent(param))
			+ '&access_token=' + escape(token) + '&format=json',
